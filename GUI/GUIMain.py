...
class myApp(QWidget, Ui_Form): # la classe reçoit le Qwidget principal ET la classe définie dans test.py obtenu avec pyuic4
	...
	def openPort(self): 
		...
		self.serial = serial.Serial( '/dev/ttyACM0', 115200,serial.EIGHTBITS, serial.PARITY_NONE, serial.STOPBITS_ONE, timeout=0.1)

		self.pb_generateGCode.setEnabled(True)
		self.lbl_status.setText("GCode non genere")
	...
	def generateGCode(self):
		...
		for xBloc in range(0, int(nbBlocX)):
			...
			for xStep in range(0,int(self.yIter)):
				...
				for i in range(0, int(self.sb_passages.value())):
					gcode += "G01 Z"+str(self.sb_axis_z.value())+"\n"
					gcode += "G28 Z"+"\n"
		...
		self.lbl_status.setText("GCode genere")
